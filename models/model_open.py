...
class Model(object):
    ...
    def build_train_proc(self):
        ...
        self.q_state = tf.contrib.layers.dropout(q_lstm_output, self.dropout_prob, is_training=self.is_training)
        ...
        mask_output = []
        cond_list = []
        self.T_w_ = tf.Variable(tf.random_uniform([self.second_lstm_dim, self.second_lstm_dim], -0.1, 0.1), name='T_W_')
        self.T_b_ = tf.Variable(tf.random_uniform([self.second_lstm_dim], -0.1, 0.1), name='T_b_')
        with tf.variable_scope("img_first_layer"):
            for time_step in range(self.input_n_frames-1):
                ...
                T = tf.nn.sigmoid(tf.layers.dense(cell_output_second,cell_output_second.shape[-1],name= "v1",reuse=tf.AUTO_REUSE)+tf.layers.dense(self.q_last_state,cell_output_second.shape[-1],name= "q1",reuse=tf.AUTO_REUSE), name='T_relu')
                if time_step != 0:
                    cell_output_second = last_cell_output_second * T + cell_output_second * (1 - T)
                last_cell_output_second = cell_output_second
        ...
        self.T_w = tf.Variable(tf.random_uniform([3 * self.decode_dim, 3 * self.decode_dim], -0.1, 0.1), name='T_W')
        self.T_b = tf.Variable(tf.random_uniform([3 * self.decode_dim], -0.1, 0.1), name='T_b')
    ...
    def generate_answer_on_training(self):
        with tf.variable_scope("decoder"):
            ...
            with tf.variable_scope("lstm") as scope:
                # -------new----------
                attention_input = layers.multihead_attention(self.v_first_lstm_output,self.q_state,
                    num_units=self.decode_dim, dropout_rate=self.dropout_prob,is_training=self.is_training)
                # -------new----------
                ...
                for i in range(self.max_n_a_words):
                    ...
                    # -------new----------
                    attention_score = tf.nn.softmax(tf.squeeze(tf.matmul(attention_input, tf.expand_dims(decoder_state,2)), axis=[2]))
                    attention_output = tf.reduce_sum(tf.multiply(attention_input, tf.expand_dims(attention_score, 2)), 1)
                    attention_decoder = attention_output # tf.matmul(attention_output,self.attention_to_decoder)
                    # -------new----------

                    # decoder : GRU with attention
                    decoder_input = tf.concat([decoder_state,attention_decoder,current_emb],axis=1)
                    T = tf.nn.sigmoid(tf.nn.xw_plus_b(decoder_input, self.T_w, self.T_b), name='T_relu')
                    if i != 0:
                        decoder_input = last_decoder_input * T + decoder_input * (1 - T)
                    last_decoder_input = decoder_input
    ...
    def generate_answer_on_testing(self):
        with tf.variable_scope("decoder"):
            ...
            with tf.variable_scope("lstm") as scope:
                # -------new----------
                attention_input = layers.multihead_attention(self.v_first_lstm_output, self.q_state,
                                                             num_units=self.decode_dim, dropout_rate=self.dropout_prob,
                ...
                for i in range(self.max_n_a_words):
                    ...
                    attention_score = tf.nn.softmax(
                        tf.squeeze(tf.matmul(attention_input, tf.expand_dims(decoder_state, 2)), axis=[2]))
                    attention_output = tf.reduce_sum(
                        tf.multiply(attention_input, tf.expand_dims(attention_score, 2)), 1)
                    attention_decoder = attention_output  # tf.matmul(attention_output,self.attention_to_decoder)
                    # -------new----------
